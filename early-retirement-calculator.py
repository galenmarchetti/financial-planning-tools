...
market_growth_rate = parsed_args[PRE_GROWTH_RATE_KEY]
...
bank_account_needed_if_retiring_in_x_years = bank_account_needed_for_last_x_year[::-1]

# Assumes you constantly contribute and grow forever
hypothetical_bank_account = []
for i in range(0, years_to_live):
    value_to_append = None
    if i == 0:
    ...
    hypothetical_bank_account.append(value_with_net_worth_change)

print(hypothetical_bank_account)

"""
# TODO Debugging
print(hypothetical_bank_account)
print(bank_account_needed_if_retiring_in_x_years)
sys.exit(99)
"""

years_to_retirement = None
for i in range(0, years_to_live):
    if hypothetical_bank_account[i] >= bank_account_needed_if_retiring_in_x_years[i]:
...
for i in range(0, years_to_live):
    ...
    if i == 0:
        account_value = hypothetical_bank_account[0]
        ...
        if i <= years_to_retirement:
            ...
            # TODO probably a cleaner way to do that, but not worrying about it for now
            if i < years_to_retirement:
                withdrawal_value = 0
            else:
                withdrawal_value = retirement_withdrawal_in_x_years[i]
